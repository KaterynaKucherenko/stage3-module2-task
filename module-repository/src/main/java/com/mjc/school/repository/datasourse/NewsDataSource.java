...
public class NewsDataSource {
    ...
    public void setNewsModelList(List<NewsModel> newsModelListFiltered) {
        this.newsModelList = newsModelListFiltered;
    }

    public NewsModel getNewsId(Long id) {
        if (id > 0 && id <= newsModelList.size()) {
            int tmp = (id.intValue());
            return newsModelList.get(tmp - 1);
        }
        return null;
    }

    public Long getLastNewsId() {
        Long idForNewNews = newsModelList.size() + 1L;
        return idForNewNews;
    }
}
